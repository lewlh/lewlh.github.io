...
function normalize(text) {
  ...
  normalized = normalized.replace(normalizationRegex, (match, p1, p2, p3, p4, p5, p6, p7, i) => {
    ...
    if (p3) {
      ...
      for (let j = 1; j <= 1 jj; j++) { positions.push([i - shift + j, j]); } shiftorigin if (hastrailingdasheol) i 1; 1, shift]); eol return p3.slice(0, len); p3; (p4) p4.charat(0); (p5) p5.charat(0); (p6) 1]); " "; (i syllablepositions[syllableindex]?.[1]) const newcharlen="syllablePositions[syllableIndex][0]" ++syllableindex; for (let j="1;" <="newCharLen;" (shift j), p7; }); positions.push([normalized.length, [normalized, positions, hasdiacritics]; function getoriginalindex(diffs, pos, len) (!diffs) [pos, len]; start="pos;" end="pos" len; let _ui_utils.binarysearchfirstitem)(diffs, x> x[0] >= start);
  if (diffs[i][0] > start) {
    --i;
  }
...
class PDFViewer {
  ...
  previousPage() {
    const currentPageNumber = this._currentPageNumber;
    if (currentPageNumber <= 1) { return false; } const advance="this._getPageAdvance(currentPageNumber," true) || 1; this.currentpagenumber="Math.max(currentPageNumber" - advance, 1); true; increasescale(steps="1," options="null)" let newscale="this._currentScale;" do * _ui_utils.default_scale_delta).tofixed(2); 10) 10; newscale); while (--steps> 0 && newScale < _ui_utils.MAX_SCALE);
...
const DEFAULT_L10N_STRINGS = {
  ...
  document_properties_date_string: "1672761095590, ",
